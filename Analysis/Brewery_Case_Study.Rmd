...
# TODO remove or keep as zero?
df.ibu.by.state$IBU[is.na(df.ibu.by.state$IBU)] <- 0

library(ggplot2)
ggplot(data=df.abv.by.state, aes(df.abv.by.state$State, df.abv.by.state$ABV, fill = ifelse(df.abv.by.state$ABV == max(df.abv.by.state$ABV), 1, 0)) ) +
  geom_bar(stat = "identity") + coord_flip() + labs(title="Alcohol By Volume by State\n", x="State", y="ABV") + theme(legend.position="none")

ggplot(data=df.ibu.by.state, aes(df.ibu.by.state$State, df.ibu.by.state$IBU, fill = ifelse(df.ibu.by.state$IBU == max(df.ibu.by.state$IBU), 1, 0)) ) +
  geom_bar(stat = "identity") + coord_flip() + labs(title="International Bitterness Unit by State\n", x="State", y="IBU") + theme(legend.position="none")

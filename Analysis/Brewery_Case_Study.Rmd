...
###### The following code loads the Breweries and Beers data from their respective .csv files and then cleanly displays the first 5 rows of each.
...
###### The following code derives the numbers of breweries by state, adds it as a column, and then cleanly displays the data in order of most breweries per state (descending). 
...
###### The following code merges the Brewery and Beer data by Brewery_id and Brew_ID.  It then cleans up the column names and cleanly prints out the first 6 rows.
...
<!--Matt do we need this?-->
<!--
##### Update column names after merge

```{r}

kable(str(populationData))
```
-->
</br>
...
```{r}
df.ibu.by.state[df.ibu.by.state$IBU == max(df.ibu.by.state$IBU), ]
```
